...
addEventListener("sync", (event) => {
	const { data } = event;
	if (!Object.hasOwn(syncEvents, data.tag)) return;
...
addEventListener("periodicsync", (event) => {
	const { data } = event;
	if (!Object.hasOwn(periodicSyncEvents, data.tag)) return;
...
addEventListener("backgroundfetchfail", backgroundFetchFailEvent);
